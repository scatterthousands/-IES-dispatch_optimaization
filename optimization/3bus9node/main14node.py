...
import config14node as config

# ==========================================
# 1. 拓扑解析与参数计算
# ==========================================
ts_data = config.get_time_series_data()
T, dt_sec, dt_h = config.SIM_SETTINGS['T'], config.SIM_SETTINGS['dt_sec'], config.SIM_SETTINGS['dt_hour']
...
print(f"--- 14节点网络 仿真设置: 步长 {dt_sec / 60:.0f} 分钟, 共 {T} 步 ---")
for cfg in config.PIPE_CONFIGS:
    ...
    print(
        f"Pipe {cfg['id']:>3} ({cfg['fr']:>3}->{cfg['to']:>3}): N={params['N']:>2}, Co={params['Co']:.2f}, Delay={params['delay_h']:.2f}h")
...
# 2. Gurobi 模型构建
# ==========================================
model = gp.Model("IES_14Node_Stable")
model.Params.NumericFocus = 1
model.Params.BarHomogeneous = 1

lim, dev, bldg, cp_kw = config.LIMITS, config.DEVICES, config.BUILDINGS, config.PHYSICS['Cp_kW']

# --- A. 变量定义 ---
# 所有的热源设备都在 N1 节点
P_chp = model.addVars(T, lb=0, ub=dev['N1']['Cap_CHP_Elec'], name="P_chp")
Q_chp = model.addVars(T, lb=0, name="Q_chp")
P_fc = model.addVars(T, lb=0, ub=dev['N1']['Cap_FC_Elec'], name="P_fc")
Q_fc = model.addVars(T, lb=0, name="Q_fc")
Q_eb = model.addVars(T, lb=0, ub=dev['N1']['Cap_EB_Heat'], name="Q_eb")
...
Ts_node = model.addVars(config.NODES, T + 1, lb=0.0, ub=120.0, name="Ts_node")
Tr_node = model.addVars(config.NODES, T + 1, lb=0.0, ub=120.0, name="Tr_node")
Ts_pipe = {p: model.addVars(Pipes[p]['N'] + 1, T + 1, lb=0.0, ub=120.0) for p in Pipes}
Tr_pipe = {p: model.addVars(Pipes[p]['N'] + 1, T + 1, lb=0.0, ub=120.0) for p in Pipes}

T_in = model.addVars(config.LOADS, T + 1, lb=0.0, ub=40.0, name="T_in")
Q_sup = model.addVars(config.LOADS, T, lb=0, name="Q_sup")
Q_slack = model.addVars(config.LOADS, T, lb=0, name="Q_slack")
T_slack = model.addVars(config.LOADS, T, lb=0, name="T_slack")
Tr_bldg = model.addVars(config.LOADS, T, lb=0.0, ub=120.0, name="Tr_bldg")
...
for t in range(T):
    # 温控上限
    for n in config.NODES: model.addConstr(Ts_node[n, t] <= lim['Ts_max'])

    model.addConstr(Q_chp[t] == P_chp[t] * dev['N1']['Alpha_CHP'])
    model.addConstr(Q_fc[t] == P_fc[t] * dev['N1']['Alpha_FC'])
    model.addConstr(Q_eb[t] == P_eb[t] * dev['N1']['Eta_EB'])
    ...
    for n in config.NODES:
        ...
        if m_ret_total > 0: model.addConstr(Tr_node[n, t] * m_ret_total == ret_enth_in)
        ...
        if n in config.SOURCES:
            ...
            # 所有供能设备都在 N1
            model.addConstr(Q_chp[t] + Q_fc[t] + Q_eb[t] == m_src * cp_kw * (Ts_node[n, t] - Tr_node[n, t]))
            ...
            model.addConstr(T_in[n, t + 1] <= lim['Tin_max'] + T_slack[n, t])
...
# GB 被移除，不再计入气耗成本
cost_fuel = gp.quicksum((P_chp[t] / 0.35 + P_fc[t] / 0.5) * eco['Cost_Gas'] * dt_h for t in range(T))
cost_grid = gp.quicksum(P_buy[t] * Price_Elec[t] * dt_h for t in range(T))
cost_pen = gp.quicksum(
    (eco['Cost_Curtail'] * P_curt[t] +
     sum(eco['Cost_Slack_Q'] * Q_slack[n, t] + eco['Cost_Slack_T'] * T_slack[n, t] + 100.0 * T_bp_slack[n, t] for n in
         config.LOADS)) * dt_h
    for t in range(T)
)
emi = gp.quicksum(
    (P_buy[t] * eco['Factor_Grid'] + (P_chp[t] / 0.35) * eco['Factor_Gas'] + (P_fc[t] / 0.5) * eco['Factor_FC']) * dt_h
    for t in range(T))
...
if model.status == GRB.OPTIMAL:
    ...
    # 1. 建筑温度
    ax1 = axes[0]
    ax1.set_title("(a) 14-Node Building Status (Loads N6, N11, N14)", fontweight='bold')
    ...
    ax1b = ax1.twinx()
    ax1b.step(t_ax, Price_Elec, where='post', color='gray', linestyle='--', label='Price')
    ax1.legend(loc='upper left');
    ax1.set_ylabel('Temp (C)')

    # 2. 热源调度 (GB移除)
    ax2 = axes[1]
    ax2.set_title("(b) Heat Dispatch at Source Node N1", fontweight='bold')
    q_chp, q_fc, q_eb = [Q_chp[t].X for t in range(T)], [Q_fc[t].X for t in range(T)], [Q_eb[t].X for t in range(T)]
    ax2.stackplot(t_ax, q_chp, q_fc, q_eb, labels=['CHP', 'Fuel Cell', 'EB'], colors=['orange', 'cyan', 'purple'],
                  alpha=0.8)
    ...
    # 4. 温度动态 (展示源端N1，中间枢纽N5，最远末端N14)
    ax4 = axes[3]
    ax4.set_title("(d) Network Temp Dynamics (Long Distance Flow: N1 -> N5 -> N14)", fontweight='bold')
    ax4.plot(t_ax, [Ts_node['N1', t].X for t in range(T)], 'r-', lw=2.5, label='Source N1')
    ax4.plot(t_ax, [Ts_node['N5', t].X for t in range(T)], 'm--', lw=2, label='Junction N5 (Mid-point)')
    ax4.plot(t_ax, [Ts_node['N14', t].X for t in range(T)], 'orange', ls='-.', lw=2,
             label='Load N14 (Farthest Terminal)')
    ax4.legend();
